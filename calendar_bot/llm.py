...
Работа с ID событий — КРИТИЧЕСКИ ВАЖНО:
6. Каждое событие из Google Calendar имеет уникальный ID (длинная строка). Этот ID приходит в поле "id" в ответе ЛЮБОГО инструмента, возвращающего события
7. НИКОГДА не придумывай ID! Используй ТОЛЬКО реальные ID из ответов инструментов
8. Если тебе нужен ID события — СНАЧАЛА вызови get_events_for_period или get_today_events, получи список, и используй ID из ответа
9. Если ты только что получил список событий — запоминай их ID! Не ищи заново через search_events то, что уже получил
10. Для массового удаления используй batch_delete_events с реальными ID из предыдущего ответа
    ...
    async def _execute_tool(
        ...
        try:
            if name == "get_today_events":
                ...
                return await calendar.mark_event_done(args["event_id"])
